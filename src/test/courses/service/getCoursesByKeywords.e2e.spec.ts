...
describe('CourseService (e2e)', () => {
    ...
    it('should get courses by keywords', async () => {
        const sampleKeywords = ['curso'];

        jest.spyOn(courseService, 'getCoursesByKeywords').mockResolvedValue({
            statusCode: 200,
            message: 'COURSESGOT',
            coursesFound: [
            ],
        ...
        const response = await courseService.getCoursesByKeywords(sampleKeywords);
