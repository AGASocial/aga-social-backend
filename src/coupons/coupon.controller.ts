...
export class CouponController {
  ...
  @ApiResponse({
    status: 400,
    description:
      'Bad Request: Invalid input, coupon already exists, invalid status, invalid discount type, asset does not exist, or expiry date is invalid',
  })
  @Post('coupons')
  async createCoupon(
    ...
    try {
      const createCouponResponse: CreateCouponResponseDto =
        await this.couponService.createNewCoupon(createCouponDto);
  ...
  @ApiParam({
    ...
    type: 'string',
    example: 'TESTCOUPON123',
  })
  @ApiBody({ type: UpdateCouponDto })
  @ApiResponse({
    status: 200,
    type: UpdateCouponResponseDto,
    description: 'Coupon updated successfully',
  })
  @ApiResponse({
    status: 400,
    description: 'Bad Request: Invalid input or coupon not found',
  })
  @Patch('coupons/:code')
  async updateCoupon(
    ...
    try {
      const updateCouponResponse: UpdateCouponResponseDto =
        await this.couponService.updateCoupon(code, updateCouponDto);
  ...
  async redeemCoupon(
    ...
    try {
      const response: RedeemCouponResponseDto =
        await this.couponService.redeemCoupon(redeemCouponDto, userId);
  ...
  @ApiOkResponse({
    ...
    type: SetCouponAsExpiredResponseDto,
  })
  @ApiBadRequestResponse({
    description: 'Bad Request: Invalid input or coupon not found',
  })
  ...
  async updateExpiredCouponsStatus(@Res() res: Response) {
    try {
      const updateStatusResponse: SetCouponAsExpiredResponseDto =
        await this.couponService.updateExpiredCouponsStatus();
  ...
  @ApiBadRequestResponse({
    description: 'Bad Request: Invalid input or coupons not found',
  })
  @Get('coupons/:code')
  async getCouponByCode(
    ...
    try {
      const couponResponse: GetCouponsResponseDto =
  ...
  async getCouponsCreatedByUser(
    ...
    try {
      const coupons: GetCouponsResponseDto =
        await this.couponService.getCouponsCreatedByUser(userId);
