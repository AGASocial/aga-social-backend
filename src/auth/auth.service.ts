...
export class AuthService {
  ...
  async firebaseLogin(logInDto: LogInDto): Promise<LogInResponseDto> {
    ...
    try {
      ...
      console.warn(
        `firebaseLogin - Error while signing in with email and password: ${error}`,
      );
      const response: LogInResponseDto = {
        status: 'error',
        code: 400,
        message:
          'There was an error processing the request. The credentials are invalid',
        data: {
          result: {},
        },
      };
    ...
    if (userCredential) {
      ...
      const sessionId = await userCredential.user.getIdToken();
  ...
  async firebaseRefresh(refreshDto: RefreshDto): Promise<RefreshResponseDto> {
    try {
      ...
      const userEmail = this.usersService.extractEmail(
      ...
      if (expiresIn <= 0) {
        const errorResponse: RefreshResponseDto = {
          ...
          code: 400,
          message:
            'There are no sessions in course or the cookies were recently refreshed',
          data: {
            result: {},
          },
        };
  ...
  async firebaseSignUp(signUpDto: SignUpDto): Promise<SignUpDtoResponse> {
    ...
    const customUserWhere: QueryFieldFilterConstraint = where(
      'username',
      '==',
      username,
    );
    ...
    try {
      ...
      if (userCredential) {
        ...
        let docReference: DocumentReference = doc(
          this.firebaseService.usersCollection,
          id,
        );
        await setDoc(docReference, newUser);

        const cachedUsers = await this.firebaseService.getCollectionData(
          'users',
        );
    ...
    try {
      ...
      if (!user) {
        const response: UpdateUserResponseDto = {
          ...
          code: 400,
          message:
            'There was an error processing the request, please write the email again.',
          data: {
            result: {},
          },
        };
  ...
  @ApiOkResponse({
    ...
    type: ChangePasswordDtoResponse,
  })
  @ApiBadRequestResponse({
    description: 'Bad Request: Invalid password or password change failed',
  })
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
  async firebaseChangePassword(
  ...
  async getUsers(): Promise<GetUsersResponseDto> {
    try {
      ...
      const getUsersDtoResponse: GetUsersResponseDto = new GetUsersResponseDto(
        'success',
        200,
        'Users retrieved successfully',
  ...
  @ApiOkResponse({
    description: 'Security answer changed successfully',
    type: ChangeSecurityAnswerDtoResponse,
  })
  @ApiBadRequestResponse({
    description: 'Bad Request: Invalid input or incorrect credentials',
  })
  async changeSecurityAnswer(
    changeSecurityAnswerDto: ChangeSecurityAnswerDto,
    jwtToken: string,
  ): Promise<ChangeSecurityAnswerDtoResponse> {
    ...
    const newHashedSecurityAnswer = await this.hashService.hashString(
      new_security_answer,
    );
  ...
  async updateUser(
    userId: string,
    newData: Partial<UpdateUserDto>,
  ): Promise<UpdateUserResponseDto> {
    try {
      const userDocumentRef: DocumentReference = doc(
      ...
      const cachedUsers = await this.firebaseService.getCollectionData('users');
      const cachedUserIndex = cachedUsers.findIndex(
        (user) => user.id === userId,
      );
  ...
  async getUsersEarnings(id: string): Promise<GetUsersEarningsResponseDto> {
    try {
      ...
      if (userQuerySnapshot.empty) {
        return new GetUsersEarningsResponseDto(
          'error',
          404,
    ...
    try {
      ...
      if (file.size > maxFileSize) {
        return new UpdateUserResponseDto(
          'error',
          400,
          'Bad request. File exceeds the maximun size of 10MB',
          {},
        );
      }
