...
export class UsersService {
  ...
  async emailChecker(email: string, isEmailWanted: boolean) {
    ...
    const customEmailWhere: QueryFieldFilterConstraint = where(
      'email',
      '==',
      email,
    );
    ...
    if (emailQuerySnapshot.empty == isEmailWanted) {
      if (isEmailWanted == true) {
        ...
        throw new BadRequestException(
          'The email is already registered in another account',
        );
      }
    }
  ...
  async searchUser(username: string) {
    ...
    const customUsernameWhere: QueryFieldFilterConstraint = where(
      'username',
      '==',
      username,
    );
  ...
  async passwordCheck(
    password: string,
    firestoreUserSnap: any,
  ): Promise<boolean> {
    const doPasswordsMatch: boolean =
      (await this.hashService.compareHashedStrings(
        password,
  ...
  @ApiOkResponse({
    description: 'Expiration timestamp extracted.',
    type: Number,
  })
  @ApiOkResponse({
    description: 'Failed to extract expiration timestamp.',
    type: null,
  })
